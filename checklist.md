...
# Multi Page Dashboard Split Checklist

- [x] 현재 `web/index.html` 단일 페이지 구조와 데이터 렌더링 범위 확인.
- [x] 공통 CSS와 JS를 새 파일로 분리.
- [x] 팀 대시보드, 자동화 루틴, 콘텐츠, 리서치 개별 HTML 페이지 생성.
- [x] `index.html`을 팀 대시보드 진입점으로 정리.
- [x] 페이지별 네비게이션과 활성 상태 연결.
- [x] 대시보드 데이터 재생성 및 테스트 실행.
- [x] 로컬 브라우저에서 네 개 페이지 렌더링 검증.
